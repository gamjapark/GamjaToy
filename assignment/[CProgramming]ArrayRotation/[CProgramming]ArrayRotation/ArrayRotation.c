...
int main() {
	...
	int degree, nHeight, nWidth;

	printf("배열의 행의 크기를 입력하세요 : "); scanf("%d", &nHeight);	//input 2D array col size
	printf("배열의 열의 크기를 입력하세요 : "); scanf("%d", &nWidth);	//input 2D array row size

	HEIGHT = nHeight; WIDTH = nWidth;

	int** myArray = Alloc2DArray(nHeight, nWidth);	//allocate 2D array
	printf("배열을 입력하세요 : \n");					//input 2D array value
	for (int h = 0; h < nHeight; h++)
		for (int w = 0; w < nWidth; w++)
			scanf("%d", &myArray[h][w]);

	Print2DArray(myArray, nHeight, nWidth, "입력한 배열 : \n");	//print 2D array

	printf("회전할 각도를 입력하세요 : ");	//input Rotataion angle
	scanf("%d", &degree);

	introtateArray(myArray, degree);		//rotate 2D array 
	Free2DArray(myArray, nHeight);			//deallocate 2D array
...
int introtateArray(int** x, int degree) {
	...
	if (rotationArray != NULL) {
		Print2DArray(rotationArray, HEIGHT, WIDTH, "회전한 배열 : \n");	//print rotation 2D array
		Free2DArray(rotationArray, HEIGHT);								//deallocatie rotation 2D array
		return 1;
	}
	return 1;
}
